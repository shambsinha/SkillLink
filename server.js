...
app.post('/signup', (req, res) => {
  let { email, username, password } = req.body; // Changed 'pass' to 'password'
  console.log('Signup request received with password:', password); // Debugging line
  dbinstance.collection('customer').find({ email: email }).toArray().then(data => {
    if (data.length > 0) {
      res.render('signup', { message: 'Email already exists' });
    } else {
      ...
      // Delete any existing OTP entries for this email
      dbinstance.collection('otps').deleteMany({ email: email }).then(() => {
        dbinstance.collection('customer').insertOne({ email: email, username: username, pass: password, role: 'customer' }).then(() => {
...
app.post('/login', (req, res) => {
  let { username, password } = req.body; // Changed 'pass' to 'password'
  console.log('Login attempt with username:', username, 'and password:', password); // Debugging line

  dbinstance.collection('customer').findOne({ username: username }).then(user => {
